...
const Showstory = ({ slides,setStoryView }) => {
  ...
  useEffect(()=>{
    const fetchBookMarkStatus=async()=>{
       ...
       axios.get(`http://localhost:3000/api/slide/isBookmarked/${slides[currentSlideIndex]._id}`)
       .then((res)=>{
  ...
  const handleBookmark=async(slideIndex)=>{
      ...
      axios.defaults.headers.common["Authorization"] = `${token}`;
      axios.post(`http://localhost:3000/api/slide/${endPoint}`,{
        slideId
      })
      .then((res)=>{
        const newBookmarkStatus = [...bookmarkStatus];
  ...
  const handleLike=async(slideIndex)=>{
       const slideId=slides[slideIndex]._id;
       axios.defaults.headers.common["Authorization"] = `${token}`;
       axios.post(`http://localhost:3000/api/slide/like`,{
          slideId
       }).then((res)=>{
          const data= res.data;
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.cardContainer}>
        <div className={styles.top}>
          ...
          <div className={styles.Topicons}>
            <IoMdClose className={styles.closeIcons} onClick={()=>setStoryView(false)}/>
        ...
        <div className={styles.bottom}>
          {bookmarkStatus[currentSlideIndex]?<FaRegBookmark className={styles.bookmark} onClick={()=>handleBookmark(currentSlideIndex)}/> :<FaBookmark className={styles.bookmark} onClick={()=>handleBookmark(currentSlideIndex)}/>}
          <div className={styles.likesOv}>
          {!likeStatus[currentSlideIndex]?<CiHeart className={styles.likes} onClick={()=>handleLike(currentSlideIndex)}/>:<FaHeart className={styles.likes} onClick={()=>handleLike(currentSlideIndex)}/>}
          <p
              style={{
                color: "white",
              }}
      ...
      <MdNavigateNext onClick={handleNextSlide} className={styles.moveIcons} />
    </div>
  );
};
